...
class FruitMap:
    def __init__(self, screen: pygame.surface.Surface, TILE_SIZE: tuple[int, int]):
        self.screen: pygame.surface.Surface = screen
            ...
            for ir, row in enumerate(data):
                ...
                for ic, fruit in enumerate(row):
                    if fruit in self.keys:
                        self.grid[ir].append(self.keys[fruit](self.screen, self.TILE_SIZE, (ic*self.TILE_W, ir*self.TILE_H)))
                    else:
